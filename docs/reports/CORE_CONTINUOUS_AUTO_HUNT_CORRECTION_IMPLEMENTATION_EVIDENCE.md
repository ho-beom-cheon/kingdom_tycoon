...
| 신규 PlayMode | 3 통과 / 0 실패 |
| 전체 EditMode | 139 통과 / 0 실패 |
| 전체 PlayMode | 46 통과 / 0 실패 / 렌더 캡처 1 의도적 생략 |
| 콘텐츠·P07~P17 정적 게이트 | 통과 |
| 신규 상시 사냥 CI 게이트 | 통과 |
| Java 25 + PostgreSQL 18 Testcontainers | 13 통과 / 0 실패 |
| Unity 구조·meta 정책 | 통과 |

전체 PlayMode 최초 실행에서는 신규 테스트가 배치 상태를 남겨 후속 P05 테스트가 영향을 받는 테스트 격리 결함을 발견했다. 신규 테스트 종료 시 전원 귀환·정산하도록 보완했다. 이후 실제 하단 `사냥터` 버튼이 기존 지역 지도만 여는 씬 이벤트 연결 결함과, 자동 사냥 화면이 생성됐지만 상위 내비게이션 Canvas 뒤에 가려지는 정렬 결함을 수동 실행에서 발견했다. 실제 `RegionMapEntryButton` 진입점을 자동 사냥 화면으로 교체하고, 구형 지도를 닫은 뒤 최상위 Canvas 레이어에 표시하도록 수정했다. 회귀 테스트는 실제 진입 컴포넌트, 루트 Canvas 정렬값, 구형 지도 비활성화를 함께 검증한다. 최종 표적 PlayMode는 3개 통과, 전체 PlayMode는 46개 통과와 렌더 캡처 1개 의도적 생략이다.
...
- 하단 `사냥터` 버튼으로 기존 지역 지도가 아닌 `상시 자동 사냥` 화면이 최상단에 열리는지 확인
