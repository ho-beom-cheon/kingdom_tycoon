...
전체 PlayMode 최초 실행에서는 신규 테스트가 배치 상태를 남겨 후속 P05 테스트가 영향을 받는 테스트 격리 결함을 발견했다. 신규 테스트 종료 시 전원 귀환·정산하도록 보완했다. 이후 실제 하단 `사냥터` 버튼이 기존 지역 지도만 여는 씬 이벤트 연결 결함도 수동 실행에서 발견해 `OpenRegions`로 교체하고 버튼 클릭 회귀 테스트를 추가했다. 최종 전체 결과는 46개 통과다.
