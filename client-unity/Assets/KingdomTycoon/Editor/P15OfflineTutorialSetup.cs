...
namespace KingdomTycoon.Editor
{
    public static class P15OfflineTutorialSetup
    {
        ...
        internal static void Capture(GameObject root, string directory, string filename, int width, int height)
        {
            ...
            Color32[] capturePixels = texture.GetPixels32(); int stride = Math.Max(1, capturePixels.Length / 4096); int distinct = capturePixels.Where((_, index) => index % stride == 0).Select(value => (value.r, value.g, value.b, value.a)).Distinct().Take(16).Count();
            ...
            if (new FileInfo(path).Length <= 20000 || distinct < 8) throw new BuildFailedException("P15_CAPTURE_FAILED: " + filename);
        }
    ...
    public static class P15CaptureGenerator
    {
        ...
        public static void Run()
        {
            ...
            nodes.Single(value => value.name == "P15_ACTION").GetComponent<TMP_Text>().text = "<b>다음 목표</b>  자동 사냥 관찰\n대상: 왕국 외곽 초원"; P15OfflineTutorialSetup.Capture(root, output, "p15_04_tutorial_active.png", 1920, 1080);
