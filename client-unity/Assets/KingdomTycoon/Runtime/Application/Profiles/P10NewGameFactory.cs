...
{
    public sealed class P10NewGameFactory : INewGameFactory
    {
        private readonly JObject template;
        public P10NewGameFactory(string templateJson) => template = StrictJson.ParseObject(templateJson ?? throw new ArgumentNullException(nameof(templateJson)));
        ...
        public JObject CreateDraft(string saveId, string profileId, DateTimeOffset now)
        {
            ...
            document["gameVersion"] = "1.0.0-p10";
            document["contentVersion"] = CompileTimeActiveContentVersionProvider.P10ContentVersion;
