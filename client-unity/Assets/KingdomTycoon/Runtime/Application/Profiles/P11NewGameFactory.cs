...
{
    public sealed class P11NewGameFactory : INewGameFactory
    {
        private readonly JObject template;
        public P11NewGameFactory(string templateJson) => template = StrictJson.ParseObject(templateJson ?? throw new ArgumentNullException(nameof(templateJson)));
        ...
        public JObject CreateDraft(string saveId, string profileId, DateTimeOffset now)
        {
            ...
            document["revision"] = 0; document["createdAtUtc"] = timestamp; document["savedAtUtc"] = timestamp;
            document["gameVersion"] = "1.0.0-p11"; document["contentVersion"] = CompileTimeActiveContentVersionProvider.P11ContentVersion;
            ...
            foreach (JObject mercenary in document["payload"]!["mercenaries"]!.Children<JObject>())
            {
                ...
                mercenary["autonomy"]!["stateStartedAtUtc"] = timestamp; mercenary["autonomy"]!["nextDecisionAtUtc"] = timestamp;
            }
