...
namespace KingdomTycoon.Presentation.Raids
{
    public sealed class RaidScreenView : MonoBehaviour
    {
        ...
        public void ShowError(string code) { Debug.LogWarning(code); resultText.text = "<color=#E17C72><b>출정 불가</b></color>\n출정 조건과 편성을 다시 확인해 주세요."; }
        ...
        private static string Rank(string id) => id switch { "RANK_APPRENTICE" => "수습", "RANK_REGULAR" => "정식", "RANK_SKILLED" => "숙련", "RANK_ELITE" => "정예", "RANK_HERO" => "영웅", "RANK_LEGEND" => "전설", _ => "등급 미정" };
        private static string Difficulty(string id) => id switch { "NORMAL" => "일반", "HARD" => "고난", "CORRUPTED" => "타락", _ => "일반" };
        private static string RaidName(string id) => id == "RAID_HYDRA" ? "늪의 히드라" : "재의 고룡";
        private static string Job(string id) => id switch { "JOB_WARRIOR" => "전사", "JOB_GUARDIAN" => "수호자", "JOB_ARCHER" => "궁수", "JOB_MAGE" => "마법사", "JOB_CLERIC" => "성직자", _ => "역할 미정" };
        private static string Part(string id) => id?.Replace("HYDRA_", string.Empty).Replace("DRAGON_", string.Empty) switch { "HEAD" => "머리", "BODY" => "몸통", "HEART" => "심장", "HORN" => "뿔", "WING" => "날개", _ => "부위" };
        private static string Behavior(string value) => value switch { "DISABLE_REGEN" => "재생 봉쇄", "REDUCE_DEFENSE" => "방어 약화", "EXPOSE_CORE" => "핵심 노출", "GROUND_BOSS" => "지상 고정", "REDUCE_ATTACK" => "공격 약화", _ => "행동 변화" };
    }
}
