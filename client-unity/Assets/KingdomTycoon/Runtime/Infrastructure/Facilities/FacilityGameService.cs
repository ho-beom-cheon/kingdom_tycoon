...
namespace KingdomTycoon.Infrastructure.Facilities
{
    public sealed class FacilityGameService : IAppService
    {
        ...
        private readonly bool p15Enabled;
        ...
        public FacilityGameService(ITrustedUtcClock clock, string newGameTemplateJson, string p04MigrationTemplateJson = null)
        {
            ...
            p15Enabled = StrictJson.ParseObject(newGameTemplateJson).Value<string>("contentVersion") == CompileTimeActiveContentVersionProvider.P15ContentVersion;
        ...
        public async Task BootstrapAsync(CancellationToken cancellationToken)
        {
            ...
            if (located.Kind == ProfileLocateKind.NONE)
            {
                ...
                INewGameFactory factory = p15Enabled ? new P15NewGameFactory(newGameTemplateJson) : p14Enabled ? new P14NewGameFactory(newGameTemplateJson) : p13Enabled ? new P13NewGameFactory(newGameTemplateJson) : p12Enabled ? new P12NewGameFactory(newGameTemplateJson) : p11Enabled ? new P11NewGameFactory(newGameTemplateJson) : p10Enabled ? new P10NewGameFactory(newGameTemplateJson) : p09Enabled ? new P09NewGameFactory(newGameTemplateJson) : p08Enabled ? new P08NewGameFactory(newGameTemplateJson) : p07Enabled ? new P07NewGameFactory(newGameTemplateJson) : p06Enabled ? new P06NewGameFactory(newGameTemplateJson) : p05Enabled ? new P05NewGameFactory(newGameTemplateJson) : new P04NewGameFactory(newGameTemplateJson);
            ...
            else if (CurrentDocument.Value<string>("contentVersion") == CompileTimeActiveContentVersionProvider.P08ContentVersion && (p09Enabled || p10Enabled || p11Enabled || p12Enabled || p13Enabled || p14Enabled || p15Enabled))
            {
            ...
            if (CurrentDocument.Value<string>("contentVersion") == CompileTimeActiveContentVersionProvider.P09ContentVersion && (p10Enabled || p11Enabled || p12Enabled || p13Enabled || p14Enabled || p15Enabled))
            {
                JObject migrated = new P09ToP10ContentMigration().Apply(CurrentDocument);
                Commit(migrated, Revision, clock.UtcNow);
            }
            if (CurrentDocument.Value<string>("contentVersion") == CompileTimeActiveContentVersionProvider.P10ContentVersion && (p11Enabled || p12Enabled || p13Enabled || p14Enabled || p15Enabled))
            {
                JObject migrated = new P10ToP11ContentMigration().Apply(CurrentDocument);
                Commit(migrated, Revision, clock.UtcNow);
            }
            if (CurrentDocument.Value<string>("contentVersion") == CompileTimeActiveContentVersionProvider.P11ContentVersion && (p12Enabled || p13Enabled || p14Enabled || p15Enabled))
            {
                JObject migrated = new P11ToP12ContentMigration().Apply(CurrentDocument);
                Commit(migrated, Revision, clock.UtcNow);
            }
            if (CurrentDocument.Value<string>("contentVersion") == CompileTimeActiveContentVersionProvider.P12ContentVersion && (p13Enabled || p14Enabled || p15Enabled))
            {
                JObject migrated = new P12ToP13ContentMigration().Apply(CurrentDocument);
                Commit(migrated, Revision, clock.UtcNow);
            }
            if (CurrentDocument.Value<string>("contentVersion") == CompileTimeActiveContentVersionProvider.P13ContentVersion && (p14Enabled || p15Enabled))
            {
                JObject migrated = new P13ToP14ContentMigration().Apply(CurrentDocument);
                Commit(migrated, Revision, clock.UtcNow);
            }
            if (CurrentDocument.Value<string>("contentVersion") == CompileTimeActiveContentVersionProvider.P14ContentVersion && p15Enabled)
            {
                JObject migrated = new P14ToP15ContentMigration().Apply(CurrentDocument);
                Commit(migrated, Revision, clock.UtcNow);
            }
            if (CurrentDocument.Value<string>("contentVersion") is not (CompileTimeActiveContentVersionProvider.P04ContentVersion or CompileTimeActiveContentVersionProvider.P05ContentVersion or CompileTimeActiveContentVersionProvider.P06ContentVersion or CompileTimeActiveContentVersionProvider.P07ContentVersion or CompileTimeActiveContentVersionProvider.P08ContentVersion or CompileTimeActiveContentVersionProvider.P09ContentVersion or CompileTimeActiveContentVersionProvider.P10ContentVersion or CompileTimeActiveContentVersionProvider.P11ContentVersion or CompileTimeActiveContentVersionProvider.P12ContentVersion or CompileTimeActiveContentVersionProvider.P13ContentVersion or CompileTimeActiveContentVersionProvider.P14ContentVersion or CompileTimeActiveContentVersionProvider.P15ContentVersion))
            {
