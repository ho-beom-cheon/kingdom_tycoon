...
- `docs/15_UI_UX_DESIGN_SYSTEM.md`와
  `docs/26_ASSET_AUDIO_LICENSE_BUDGET.md`를 화면·에셋 구현 계약으로 취급한다.
- 외부 상용 게임은 완성도와 가독성을 판단하는 참고 기준일 뿐이며, 타사의
  그래픽·UI·명칭·연출을 복제하지 않는다.
- P08~P14는 상업 이용이 확인된 무료·기보유 에셋을 우선하고, P15에서 전체
  UX와 시각 스타일을 통합한다.
- 유료 에셋은 P17 초입의 상용화 판단과 사용자 명시 승인이 모두 있기 전에는
  구매·도입하지 않는다. 승인 후에도 총액은 200,000원을 넘지 않는다.
- 외부 에셋은 프로젝트에 넣기 전에 `asset_register.csv`에 출처·가격·라이선스·
  상업 이용·수정·크레딧 조건을 기록한다.
- 에셋 교체가 도메인 로직이나 Save를 변경하지 않도록 ID·카탈로그 기반으로
  참조하고, 서로 다른 PPU·시점·외곽선·광원 방향을 무분별하게 섞지 않는다.
